...
第十二阶段已经完成后端最小闭环。下一步从第十三阶段开始：RAG 检索问答与引用。不要把工具调用或异步任务提前混进聊天主链路。
...
> 当前状态：已完成后端最小闭环。已新增 `knowledge_bases`、`knowledge_documents`、`knowledge_chunks`，支持本地 `md/txt/json/pdf` 导入、切片、`local-hash-embedding-v1` embedding、SQLite 本地向量检索和自动化测试。
